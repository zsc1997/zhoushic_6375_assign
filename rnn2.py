...
class BiDirectionalGRU(nn.Module):
    def __init__(self, input_dim, h, output_dim):
        ...
        self.gru1 = nn.GRU(128, h, num_layers=1, batch_first=True, bidirectional=True)
        self.dropout1 = nn.Dropout(0.4)  
        self.fc = nn.Linear(h * 2, output_dim)  # Fully connected layer without flattening
    ...
    def forward(self, input_sequence):
        ...
        # GRU layer
        gru_out, _ = self.gru1(embedded)
        gru_out = self.dropout1(gru_out)

        # Use the last output of GRU as the sequence representation
        gru_out = gru_out[:, -1, :]

        # Fully connected layer, return logits (no sigmoid)
...
if __name__ == "__main__":
    ...
    model = BiDirectionalGRU(input_dim=len(vocab), h=args.hidden_dim, output_dim=5)
